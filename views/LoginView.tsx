...
export const LoginView: React.FC<{ onLogin: (email: string) => void, onBack: () => void }> = ({ onLogin, onBack }) => {
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="bg-white w-full max-w-md p-8 md:p-10 rounded-[2.5rem] shadow-2xl shadow-indigo-100 border border-slate-100">
        ...
        <p className="mt-8 text-center text-slate-400 text-sm">
          Don't have an account? <button className="text-[#6366F1] font-bold hover:underline">Sign up</button>
        </p>
      </div>
    </div>
  );
