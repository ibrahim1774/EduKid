...
export const DashboardView: React.FC<DashboardProps> = ({ state, setState, onViewWorksheet, onAddChild }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-16 bg-[#F7F9FC]">
      ...
      {/* Subject Tab Bar - Chip style, scrollable on mobile */}
      <div className="flex items-center gap-2 mb-8 overflow-x-auto no-scrollbar pb-2 -mx-4 px-4 md:mx-0 md:px-0">
        {subjects.map(sub => (
          <button
            key={sub}
            onClick={() => setActiveSubject(sub)}
            className={`px-8 py-3 rounded-full font-bold whitespace-nowrap transition-all border-2 flex items-center gap-2 ${activeSubject === sub ? 'bg-[#1A1F3A] text-white border-[#1A1F3A] shadow-md' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200 hover:bg-slate-50'}`}
          >
            {sub === Subject.Math && <BookOpen size={18} />}
            {sub === Subject.Reading && <FileText size={18} />}
            {sub === Subject.Writing && <FileText size={18} />}
            {sub === Subject.Science && <Sparkles size={18} />}
            {sub === Subject.History && <Clock size={18} />}
            {sub}
          </button>
        ))}
      </div>
        ...
        <div className="lg:col-span-8 space-y-12">
            ...
            {todayWorksheet ? (
              ...
              <div className="bg-white p-10 md:p-16 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col items-center text-center">
                ...
                <button 
                  ...
                  className="w-full max-w-md bg-[#6C63FF] text-white py-5 rounded-2xl font-bold text-xl flex items-center justify-center gap-3 transition-all hover:shadow-2xl shadow-indigo-100 active:scale-95"
                >
          ...
          <div className="space-y-6">
             <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{filteredWorksheets.length} Total</span>
             </div>
             <div className="space-y-4">
                {filteredWorksheets.filter(w => w.date !== new Date().toLocaleDateString()).length > 0 ? filteredWorksheets.filter(w => w.date !== new Date().toLocaleDateString()).map(ws => (
                  <div key={ws.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 hover:shadow-md transition-all">
                    <div className="flex items-center gap-5 w-full overflow-hidden">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">{ws.date}</p>
                      </div>
                    </div>
                  ...
                  <div className="p-16 border-2 border-dashed border-slate-200 rounded-[2.5rem] text-center text-slate-400 font-bold bg-white/50">
                    No past {activeSubject} worksheets found.
                  </div>
                )}
             </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-indigo-50 relative overflow-hidden">
             ...
             <div className="relative z-10">
                ...
                <p className="text-slate-600 leading-relaxed font-medium text-sm">
                   Switch subjects using the top tabs to access {activeChild.name}'s full curriculum. Each area includes daily practice tailored to their current grade level.
                </p>
                <div className="mt-6 flex items-center gap-3">
                   ...
                   <span className="text-xs font-bold text-[#6C63FF] uppercase tracking-widest">Dashboard Tip</span>
                </div>
             </div>
          </div>
